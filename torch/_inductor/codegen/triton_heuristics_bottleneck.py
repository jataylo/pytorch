...
class BottleneckAnalysis:
    ...
    def _get_device_constants() -> Dict[str, float]:
        ...
        Three-level fallback so this function never raises:
          1. ArchitectureConfig lookup table (reliable for all known AMD GPUs)
        ...
        try:
            ...
            # --- Memory bandwidth: lookup table first, formula fallback ---
            # Import the lookup tables from the arch module to avoid duplicating them.
            try:
                from torch._inductor.codegen.triton_heuristics_arch import (
                    _KNOWN_HBM_BANDWIDTH_GB_S,
                    _KNOWN_L3_CACHE_BYTES,
                    _gcn_arch_prefix,
                )
                arch_prefix = _gcn_arch_prefix(props)
                memory_bandwidth_gb_s = float(
                    _KNOWN_HBM_BANDWIDTH_GB_S.get(arch_prefix, 0) or 0
                )
            ...
            if memory_bandwidth_gb_s <= 0:
                if hasattr(props, 'memory_bandwidth_gb_s'):
                    ...
                    try:
                        mem_clk_hz = props.memoryClockRate * 1e3
            ...
            # --- Compute throughput ---
            # AMD CDNA2+: 4 SIMD units × 64 FP32 lanes = 256 FP32 ops/CU/clock.
            # NVIDIA Ampere: 128 CUDA cores/SM × 2 FP32/core/clock = 256.
            if hasattr(props, 'compute_throughput_tflops'):
                ...
                try:
                    ...
                    # 256 FP32 MACs/CU/clock for both AMD CDNA2+ and NVIDIA Ampere+.
                    # Older AMD (CDNA1/gfx908): 128 ops/CU.  RDNA: variable.
                    if is_hip and arch_prefix and arch_prefix < 'gfx908':
                        ops_per_cu_clk = 128
                    else:
                        ops_per_cu_clk = 256
                    compute_tflops = max(1.0, (num_cus * ops_per_cu_clk * clk_hz) / 1e12)
                except Exception:
    ...
    def estimate_memory_time_us(total_bytes: int,
        ...
        Four-tier cache hierarchy (L1 → L2 → L3 → HBM):

          L1 (per-CU, 32 KB): true hit only when num_blocks ≤ num_CUs so each
            CU's working set is resident.  With more blocks, data is replicated
            across CUs and must come from L2 or beyond.

          L2 (shared, ~4 MB): broadcast tensors reused across all blocks at
            ~1 TB/s.  For large problems this tier is bypassed.

          L3 / Infinity Cache (AMD CDNA3+, 256 MB): acts as a victim cache for
            L2 misses.  Data that fits entirely in L3 is served at ~15 TB/s,
            dramatically faster than HBM.  Only present on gfx940/gfx942/gfx950.
        ...
        l3_cache_size         = dc.get('l3_cache_size', 0)
        ...
        if total_bytes <= l2_cache_size:
            l2_bw = 1000.0   # ~1 TB/s on AMD CDNA
            return total_bytes / (l2_bw * 1e3)

        # ── L3 / Infinity Cache tier (AMD CDNA3+ only) ────────────────────────
        # Data that fits in L3 (e.g. most weight matrices in inference) is served
        # at far higher bandwidth than HBM, avoiding the roofline being computed
        # on the wrong axis for workloads where weights are L3-resident.
        if l3_cache_size > 0 and total_bytes <= l3_cache_size:
            try:
                from torch._inductor.codegen.triton_heuristics_arch import _L3_BANDWIDTH_GB_S
                l3_bw = _L3_BANDWIDTH_GB_S
            except Exception:
                l3_bw = 15_000.0
            return total_bytes / (l3_bw * 1e3)

        # ── HBM tier ──────────────────────────────────────────────────────────
        if has_broadcast:
            num_inputs      = problem_metadata.get('num_inputs', 2)
            broadcast_bytes = total_bytes / max(num_inputs, 2)
            # Check if the broadcast tensor fits in L3 (if present) or L2.
            cache_fits = (
                (l3_cache_size > 0 and broadcast_bytes <= l3_cache_size)
                or broadcast_bytes <= l2_cache_size
            )
            if cache_fits:
                effective_bw        = memory_bandwidth_gb_s * 0.8
