...
#
# VENDORED, DO NOT EDIT BY HAND. Refresh with `refresh_vendored.py`.
#   upstream: xinya-flydsl `kernels/common/buffer_ops.py`
#   branch:   xinyazhang/sdpa-gfx950-feature-bwd
#   commit:   c36f1a7e71321a004d390f1522907c17cdfacba1  (2026-08-25)
#
# Why this is vendored at all: through FlyDSL 0.2.4 these helpers shipped *in* the
# library as `flydsl.expr.buffer_ops`. In 0.3.1 that module was deleted and the same
# code now lives kernel-side, which is where callers are expected to carry it. Our
# kernels have to build against both, so `buffer_ops.py` picks the library copy when
# it exists and this one otherwise.
