...
#include <ATen/hip/HIPSparseDescriptors.h>
#include <ATen/native/LinearAlgebraUtils.h>
#include <ATen/native/hip/MiscUtils.h>
#include <ATen/native/sparse/SparseBlasImpl.h>
#include <ATen/native/sparse/hip/SparseBlasImpl.h>
...
void block_sparse_triangular_solve_vec(
  ...
  const hipsparseDirection_t block_layout = A.values().is_contiguous()
  ...
  hipsparseOperation_t opA = transpose ? HIPSPARSE_OPERATION_TRANSPOSE
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      X.scalar_type(), "block_sparse_triangular_solve_vec", [&] {
        ...
        if (!unitriangular) {
          ...
          hipsparseStatus_t status = hipsparseXbsrsv2_zeroPivot(handle, info.descriptor(), &first_zero_diag_idx);
...
void block_sparse_triangular_solve_mat(
  ...
  hipsparseOperation_t opX = HIPSPARSE_OPERATION_NON_TRANSPOSE;
  hipsparseOperation_t opA = transpose ? HIPSPARSE_OPERATION_TRANSPOSE
                                      : HIPSPARSE_OPERATION_NON_TRANSPOSE;
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      X.scalar_type(), "block_sparse_triangular_solve_vec", [&] {
        ...
        if (!unitriangular) {
          int first_zero_diag_idx = -1;
          hipsparseStatus_t status = hipsparseXbsrsm2_zeroPivot(handle, info.descriptor(), &first_zero_diag_idx);
          if (status == HIPSPARSE_STATUS_ZERO_PIVOT) {
            X_->fill_(NAN);
            return;
          }
...
void block_sparse_mv(
  ...
  const hipsparseDirection_t block_layout = mat.values().is_contiguous()
      ? HIPSPARSE_DIRECTION_ROW
      : HIPSPARSE_DIRECTION_COLUMN;
...
void block_sparse_mm(
  ...
  const hipsparseDirection_t block_layout = mat1.values().is_contiguous()
      ? HIPSPARSE_DIRECTION_ROW
      : HIPSPARSE_DIRECTION_COLUMN;
  ...
  hipsparseOperation_t opB = is_mat2_row_major
...
void spmm(
  ...
  hipsparseOperation_t opA = HIPSPARSE_OPERATION_NON_TRANSPOSE;
  hipsparseOperation_t opB = transpose_B ? HIPSPARSE_OPERATION_TRANSPOSE
                                        : HIPSPARSE_OPERATION_NON_TRANSPOSE;
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
      ...
      [&] {
        ...
        hipDataType compute_type = at::cuda::getCudaDataType<opmath_t>();
        auto handle = at::cuda::getCurrentCUDASparseHandle();
...
void addmv_out_sparse_csr(
...
#else
  hipsparseSpMVAlg_t alg = CUSPARSE_SPMV_ALG_DEFAULT;
#endif

  // SpMV doesn't support uniform precision computation
  // For float16/bfloat16 inputs compute_type must be HIP_R_32F
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      ...
      [&] {
        ...
        size_t buffer_size;
        TORCH_CUDASPARSE_CHECK(hipsparseSpMV_bufferSize(
            handle,
            opA,
            &alpha_,
            descA.descriptor(),
        ...
        TORCH_CUDASPARSE_CHECK(hipsparseSpMV(
            handle,
            opA,
            &alpha_,
            descA.descriptor(),
...
// Let's extract manually the nnz from the C_crow_indices
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      C.scalar_type(), "add_out_sparse_csr_cuda_impl", [&] {
        ...
        auto handle = at::cuda::getCurrentCUDASparseHandle();
        TORCH_CUDASPARSE_CHECK(hipsparseSetPointerMode(handle, HIPSPARSE_POINTER_MODE_HOST));
...
void sampled_addmm_out_sparse_csr(
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
      ...
      [&] {
        ...
        // ** On entry to hipsparseSDDMM_bufferSize(): batched SDDMM is not supported
        ...
        for (const auto i : c10::irange(batchCount(A))) {
          ...
          TORCH_CUDASPARSE_CHECK(hipsparseSDDMM_bufferSize(
              handle,
              opA,
              opB,
              &alpha_,
          ...
          TORCH_CUDASPARSE_CHECK(hipsparseSDDMM_preprocess(
              handle,
              opA,
              opB,
              &alpha_,
          ...
          TORCH_CUDASPARSE_CHECK(hipsparseSDDMM(
              handle,
              opA,
              opB,
              &alpha_,
