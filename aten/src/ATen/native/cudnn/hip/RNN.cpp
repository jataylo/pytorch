...
inline bool use_rnn_persist_small_h(
  ...
  if (forward) {
    if (rnn.mode == miopenRNNRELU || rnn.mode == miopenRNNTANH) {
      return rnn.hidden_size <= 384;
    }
    if (rnn.mode == miopenLSTM || rnn.mode == miopenGRU) {
...
TORCH_HIP_CPP_API std::tuple<Tensor, std::vector<Tensor>>
...
struct DropoutState {
  ...
  std::mutex mutex;
#if !defined(USE_ROCM)
  // hipStreamGetCaptureInfo will never give back a capture id of 0, so 0 can
  ...
  void lock() {
    ...
    if (event) {
      ...
      AT_CUDA_CHECK(hipStreamGetCaptureInfo(
          hip::getCurrentHIPStreamMasqueradingAsCUDA(), &status, &capture_id_last_lock));
  ...
  void unlock() {
    if (event) {
      ...
      hipStreamCaptureStatus status;
      AT_CUDA_CHECK(hipStreamGetCaptureInfo(
          hip::getCurrentHIPStreamMasqueradingAsCUDA(), &status, &capture_id_last_unlock));
      if (status == hipStreamCaptureStatus::hipStreamCaptureStatusNone) {
