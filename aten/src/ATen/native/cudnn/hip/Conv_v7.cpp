...
struct Workspace {
  Workspace(size_t size) : size(size), data(NULL) {
    ...
    data = c10::hip::HIPCachingAllocatorMasqueradingAsCUDA::raw_alloc(size);
  }
  ...
  ~Workspace() {
    if (data) {
      c10::hip::HIPCachingAllocatorMasqueradingAsCUDA::raw_delete(data);
    }
  }
...
size_t getMaxWorkspaceSize(
  ...
  const auto device = c10::hip::current_device();
  // For the native allocator, retrieves the size of the largest unused block.
  // For hipMallocAsync, see c10/cuda/CUDAMallocAsync.cpp:cacheInfo for
  // details.
  c10::hip::HIPCachingAllocatorMasqueradingAsCUDA::cacheInfo(device, &max_block_size);
  ...
  void try_all(std::function<void(const perf_t& perf)> f) {
    ...
    if (!only_use_default && cache.find(args.params, &algoPerf)) {
      try {
        ...
        std::ignore = hipGetLastError(); // clear CUDA error
      }
    }
