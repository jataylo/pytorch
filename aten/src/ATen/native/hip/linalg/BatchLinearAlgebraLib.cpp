...
#include <ATen/native/hip/linalg/BatchLinearAlgebraLib.h>
...
static hipsolverOperation_t to_cublas(TransposeType trans) {
  switch (trans) {
    case TransposeType::NoTranspose: return HIPSOLVER_OP_N;
    case TransposeType::Transpose: return HIPSOLVER_OP_T;
    case TransposeType::ConjTranspose: return HIPSOLVER_OP_C;
  }
...
void apply_ldl_factor_cusolver(
  ...
  auto uplo = upper ? HIPSOLVER_FILL_MODE_UPPER : HIPSOLVER_FILL_MODE_LOWER;
...
static void apply_svd_cusolver_gesvdj(const Tensor& A, const Tensor& U, const Tensor& S, const Tensor& V,
  ...
  TORCH_CUSOLVER_CHECK(hipsolverDnXgesvdjSetTolerance(gesvdj_params, std::numeric_limits<scalar_t>::epsilon()));
  TORCH_CUSOLVER_CHECK(hipsolverDnXgesvdjSetMaxSweeps(gesvdj_params, cusolver_gesvdj_max_sweeps));
...
static void apply_cholesky_cusolver_potrf_looped(const Tensor& self_working_copy, bool upper, const Tensor& infos) {
  auto handle = at::cuda::getCurrentCUDASolverDnHandle();
  const auto uplo = upper ? HIPSOLVER_FILL_MODE_UPPER : HIPSOLVER_FILL_MODE_LOWER;
...
static void apply_ormqr(const Tensor& input, const Tensor& tau, const Tensor& other, bool left, bool transpose) {
  auto side = left ? HIPSOLVER_SIDE_LEFT : HIPSOLVER_SIDE_RIGHT;
  auto trans = transpose ? (input.is_complex() ? HIPSOLVER_OP_C : HIPSOLVER_OP_T) : HIPSOLVER_OP_N;
...
static void apply_syevd(const Tensor& values, const Tensor& vectors, const Tensor& infos, bool upper, bool compute_eigenvectors) {
  ...
  hipsolverFillMode_t uplo = upper ? HIPSOLVER_FILL_MODE_UPPER : HIPSOLVER_FILL_MODE_LOWER;
...
static void apply_syevj_batched(const Tensor& values, const Tensor& vectors, const Tensor& infos, bool upper, bool compute_eigenvectors) {
  ...
  hipsolverDnParams_t syev_params;
...
void apply_xgeev(const Tensor& values, const Tensor& vectors, const Tensor& input, const Tensor& infos, bool compute_eigenvectors) {
  ...
  hipsolverDnParams_t params = nullptr;
  TORCH_CUSOLVER_CHECK(cusolverDnCreateParams(&params));
  ...
  const int ldvl = 1; // ldvl >= 1 if jobvl = HIPSOLVER_EIG_MODE_NOVECTOR
  hipsolverEigMode_t jobvl = HIPSOLVER_EIG_MODE_NOVECTOR;

  hipsolverEigMode_t jobvr;
  int ldvr;
  if (compute_eigenvectors) {
    ldvr = n; // ldvr >= n if jobvr = HIPSOLVER_EIG_MODE_VECTOR
    jobvr = HIPSOLVER_EIG_MODE_VECTOR;
  }
  else {
    ldvr = 1; // ldvr >= 1 if jobvr = HIPSOLVER_EIG_MODE_NOVECTOR
    jobvr = HIPSOLVER_EIG_MODE_NOVECTOR;
  }
