...
__host__ void scan_outer_dim_with_indices(
  ...
  tensor_kernel_scan_outer_dim_with_indices<scalar_t><<<grid, threads, 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA()>>>(
...
__host__ void scan_outer_dim(const TensorBase& self, const TensorBase& result,
  ...
  tensor_kernel_scan_outer_dim<scalar_t, uint32_t><<<grid, threads, 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA()>>>(
    result.mutable_data_ptr<scalar_t>(), self.const_data_ptr<scalar_t>(),
    num_orows, num_irows, row_size, init, binary_op);
  } else  {
  tensor_kernel_scan_outer_dim<scalar_t, size_t><<<grid, threads, 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA()>>>(
    result.mutable_data_ptr<scalar_t>(), self.const_data_ptr<scalar_t>(),
    num_orows, num_irows, row_size, init, binary_op);
  }
  C10_HIP_KERNEL_LAUNCH_CHECK();
