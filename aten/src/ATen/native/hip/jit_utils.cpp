...
#include <ATen/native/hip/reduction_template.cuh>
...
// TODO: try making the hipCtx_t thread local to see if that improves performance - why is this slow?
void initializeCudaContext() {
  ...
  if (!pctx) {
    std::unique_lock<std::mutex> cudaFreeMutexLock(
        *(c10::hip::getFreeMutex()));
    AT_CUDA_CHECK(hipFree(nullptr));
  }
}
...
NvrtcFunction jit_pwise_function(
  ...
  if (cache_dir.has_value()) {
    ...
    if (read_stream.fail()) {
      ...
      AT_CUDA_DRIVER_CHECK(nvrtc.hipModuleLoadData(&(compiled_kernel_.module), buffer.data()));
  ...
  if (compilation_result != HIPRTC_SUCCESS) {
    ...
    AT_CUDA_NVRTC_CHECK(nvrtc.hiprtcGetProgramLog(program, &log[0]));
