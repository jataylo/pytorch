...
bool _hasPrimaryContext(DeviceIndex device_index) {
  ...
  int ctx_is_active = 0;
  AT_CUDA_DRIVER_CHECK(nvrtc().hipDevicePrimaryCtxGetState(device_index, &ctx_flags, &ctx_is_active));
  return ctx_is_active == 1;
}

// Register hasPrimaryContext back to c10::hip
struct _Initializer {
  _Initializer() {
      c10::hip::_internal::setHasPrimaryContext(_hasPrimaryContext);
  }
  ~_Initializer() {
      c10::hip::_internal::setHasPrimaryContext(nullptr);
  }
} initializer;
} // anonymous namespace
...
void CUDAHooks::init() const {
  ...
  const auto num_devices = c10::hip::device_count_ensure_non_zero();
  c10::hip::HIPCachingAllocatorMasqueradingAsCUDA::init(num_devices);
...
DeviceIndex current_device() {
  ...
  hipError_t err = c10::hip::GetDevice(&device);
  if (err == hipSuccess) {
    return device;
  }
  return -1;
...
std::string CUDAHooks::showConfig() const {
  ...
  AT_CUDA_CHECK(hipRuntimeGetVersion(&runtimeVersion));
