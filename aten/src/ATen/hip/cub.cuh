...
#define CUB_WRAPPER(func, ...) do {                                       \
  ...
  auto& caching_allocator = *::c10::hip::HIPCachingAllocatorMasqueradingAsCUDA::get();    \
...
inline void segmented_sort_pairs(
  ...
  if (descending) {
    CUB_WRAPPER(NO_ROCM(at_cuda_detail)::hipcub::DeviceSegmentedRadixSort::SortPairsDescending,
    ...
    CUB_WRAPPER(NO_ROCM(at_cuda_detail)::hipcub::DeviceSegmentedRadixSort::SortPairs,
      keys_in_, keys_out_, values_in, values_out,
      num_elements, num_segments, begin_offsets, end_offsets,
      begin_bit, end_bit, c10::hip::getCurrentHIPStreamMasqueradingAsCUDA());
  }
  ...
  NumSelectedIteratorT num_selected, int64_t num_input_items)
{
  ...
  CUB_WRAPPER(NO_ROCM(at_cuda_detail)::hipcub::DeviceSelect::UniqueByKey,
    keys_in, values_in, keys_out_, values_out, num_selected, num_input_items, c10::hip::getCurrentHIPStreamMasqueradingAsCUDA());
}
...
inline void inclusive_scan(InputIteratorT input, OutputIteratorT output, ScanOpT scan_op, int64_t num_items) {
  ...
  CUB_WRAPPER(NO_ROCM(at_cuda_detail)::hipcub::DeviceScan::InclusiveScan,
      input,
      output,
      scan_op,
      size_cub,
  ...
  for (int64_t i = max_cub_size; i < num_items; i += max_cub_size) {
    ...
    impl::transform_vals<<<1, 1, 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA()>>>(
        output + i - 1,
        input + i,
        first_elem_ptr,
        scan_op);
    C10_HIP_KERNEL_LAUNCH_CHECK();
    CUB_WRAPPER(NO_ROCM(at_cuda_detail)::hipcub::DeviceScan::ExclusiveScan,
        input + i + 1,
        output + i,
        scan_op,
...
inline void inclusive_deterministic_scan(const scalar_t *  input, scalar_t * output, ScanOpT scan_op, int64_t num_items) {
  ...
  <<<grid_size, BLOCK_THREADS, 0, at::hip::getCurrentHIPStreamMasqueradingAsCUDA()>>>(
...
#if !defined(USE_ROCM)
  CUB_WRAPPER(at_cuda_detail::hipcub::DeviceScan::InclusiveSumByKey,
      keys, input, output, num_items, NO_ROCM(::cuda)::std::equal_to<>(), at::hip::getCurrentHIPStreamMasqueradingAsCUDA());
#else
  CUB_WRAPPER(hipcub::DeviceScan::InclusiveSumByKey,
      keys, input, output, num_items, hipcub::Equality(), at::hip::getCurrentHIPStreamMasqueradingAsCUDA());
#endif
}
...
inline void inclusive_scan_by_key(KeysInputIteratorT keys, ValuesInputIteratorT input, ValuesOutputIteratorT output, ScanOpT scan_op, int64_t num_items) {
  TORCH_CHECK(num_items <= std::numeric_limits<int>::max(),
    "cub InclusiveSumByKey does not support more than INT_MAX elements");
#if !defined(USE_ROCM)
  CUB_WRAPPER(at_cuda_detail::hipcub::DeviceScan::InclusiveScanByKey,
      keys, input, output, scan_op, num_items, NO_ROCM(::cuda)::std::equal_to<>(), at::hip::getCurrentHIPStreamMasqueradingAsCUDA());
#else
  CUB_WRAPPER(hipcub::DeviceScan::InclusiveScanByKey,
      keys, input, output, scan_op, num_items, hipcub::Equality(), at::hip::getCurrentHIPStreamMasqueradingAsCUDA());
#endif
}
...
void reduce(InputIteratorT input, OutputIteratorT output, int64_t num_items, ReductionOpT op, T init) {
  ...
  CUB_WRAPPER(
      NO_ROCM(at_cuda_detail)::hipcub::DeviceReduce::Reduce,
