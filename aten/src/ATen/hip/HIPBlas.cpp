...
#include <ATen/hip/detail/BLASConstants.h>
...
hipblasOperation_t _cublasOpFromChar(char op) {
...
class CuBlasLtMatmulDescriptor : public CuBlasLtDescriptor<
 ...
 public:
  ...
  template <typename T>
  void setAttribute(hipblasLtMatmulDescAttributes_t attr, const T value) {
...
class CuBlasLtMatmulPreference : public CuBlasLtDescriptor<
 ...
 public:
  ...
  template <typename T>
  void setAttribute(hipblasLtMatmulPreferenceAttributes_t attr, const T value) {
...
#endif
  if (lie_to_cublaslt) {
     CuBlasLtMatrixLayout FakeBdesc(abType, k, 2, ldb, opb == HIPBLAS_OP_T);
...
#endif
  const auto epilogue = [&]() -> hipblasLtEpilogue_t {
    ...
    switch (activation) {
      case GEMMAndBiasActivationEpilogue::RELU:
        return bias ? HIPBLASLT_EPILOGUE_RELU_BIAS : HIPBLASLT_EPILOGUE_RELU;
      case GEMMAndBiasActivationEpilogue::GELU:
        return bias ? HIPBLASLT_EPILOGUE_GELU_BIAS : HIPBLASLT_EPILOGUE_GELU;
      default:
        return bias ? HIPBLASLT_EPILOGUE_BIAS : HIPBLASLT_EPILOGUE_DEFAULT;
    }
  }();
...
void scaled_gemm(
  ...
  float beta_val = 0.0;
#ifndef USE_ROCM
  // Note: unused, but hipblasLtMatmul requires a C pointer that is not result_ptr or nullptr
  ...
  if (alpha.has_value()) {
    ...
    if (a.is_cuda()) {
      ...
      auto pointer_mode = HIPBLASLT_POINTER_MODE_DEVICE;
      computeDesc.setAttribute(HIPBLASLT_MATMUL_DESC_POINTER_MODE, pointer_mode);
