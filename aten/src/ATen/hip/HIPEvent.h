...
// EventPool - Thread-safe pool of CUDA events to avoid expensive hipEventCreate
// calls. hipEventCreate when concurrently invoked from multiple threads can be
 ...
 public:
  ...
  CUDAEventPtr get(const DeviceIndex device) {
    ...
    if (device < 0 || device >= (DeviceIndex)pools_.size()) {
      ...
      return CUDAEventPtr(
        std::make_unique<CUDAEvent>(hipEventDisableTiming).release(), deleter);
    }
    ...
    return CUDAEventPtr(
        std::make_unique<CUDAEvent>(hipEventDisableTiming).release(),
        destructor);
  }
  ...
  void init_num_events(const size_t num_events) {
    for (DeviceIndex device_idx = 0; device_idx < at::cuda::device_count(); ++device_idx) {
        HIPGuardMasqueradingAsCUDA device_guard(device_idx);
