...
bool get_p2p_access(c10::DeviceIndex dev, c10::DeviceIndex dev_to_access) {
  ...
  int result = 0;
  C10_HIP_CHECK(hipDeviceCanAccessPeer(&result, dev, dev_to_access));
  cache = result ? 1 : 0;
  if (cache) {
    HIPCachingAllocatorMasqueradingAsCUDA::enablePeerAccess(dev, dev_to_access);
  }
...
namespace {
#if !defined USE_ROCM && defined TORCH_HIP_VERSION && TORCH_HIP_VERSION >= 12040 && defined PYTORCH_C10_DRIVER_API_SUPPORTED

uint32_t get_nvml_device(c10::DeviceIndex dev) {
...
bool isFabricSupported() {
  ...
  size_t granularity{};
  const auto driver_api = c10::hip::DriverAPI::get();
  ...
  // 3. check import
  hipMemGenericAllocationHandle_t import_handle = 0;
