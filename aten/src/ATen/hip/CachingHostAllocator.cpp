...
class EventPool {
 public:
  ...
  Event get(DeviceIndex device) {
    ...
    // destruction.
    return Event(
        std::make_unique<at::cuda::CUDAEvent>(hipEventDisableTiming).release(),
        destructor);
  }
...
using Block = HostBlock<HIPStreamMasqueradingAsCUDA>;

struct CUDACachingHostAllocatorImpl
    : public CachingHostAllocatorImpl<HIPStreamMasqueradingAsCUDA, EventPool::Event> {
 private:
  ...
  void allocate_host_memory_slowpath(size_t size, void** ptr) {
    ...
    auto primary_ctx_device_index =
        c10::hip::getDeviceIndexWithPrimaryContext();
    ...
    bool use_register = c10::hip::HIPCachingAllocator::HIPAllocatorConfig::pinned_use_hip_host_register();
    if (use_register) {
      allocWithCudaHostRegister(ptr, size);
    } else {
      // Use hipHostMalloc for allocating pinned memory (global lock in driver)
      C10_HIP_CHECK(hipHostMalloc(ptr, size, hipHostMallocDefault));
    }

    auto end = std::chrono::steady_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end - start);

    // Update the statistics on the time spent on hipHostMalloc/hostRegister
    {
  ...
  void free_block_slowpath(Block* block) {
    ...
    // However, allocations using hipHostRegister should use corresponding
    // hipHostUnregister and similarly for hipHostMalloc / hipHostFree.
    void* ptr = block->ptr_;
    ...
    if (use_register) {
      AT_CUDA_CHECK(hipHostUnregister(ptr));
    ...
    // Update the statistics on the time spent on hipHostFree/hostUnregister
    {
      std::lock_guard<std::mutex> g(stats_.timing_mutex_);
  ...
  void record_stream(
      ...
      HIPStreamMasqueradingAsCUDA stream) override {
  ...
  PinnedReserveSegment& get_reserve_segment() {
    static auto reserve_segment = [&]() {
      if (c10::hip::HIPCachingAllocator::HIPAllocatorConfig::pinned_reserve_segment_size_mb() > 0) {
        void *ptr;
        size_t sz = c10::hip::HIPCachingAllocator::HIPAllocatorConfig::pinned_reserve_segment_size_mb() * 1024 * 1024;
  ...
  TaskThreadPool* getThreadPool() {
    static TaskThreadPool* pool = new TaskThreadPool(
        static_cast<int>(c10::hip::HIPCachingAllocator::HIPAllocatorConfig::
  ...
  void allocWithCudaHostRegister(void** ptr, size_t roundSize) {
    ...
    // hipHostRegister with GPU mapping flags to lock the pages, so we
    ...
    *ptr = std::malloc(roundSize);
    ...
    size_t numMapThreads = c10::hip::HIPCachingAllocator::
    ...
    // Register the mapped pages using hipHostRegister
    AT_CUDA_CHECK(
        hipHostRegister(*ptr, roundSize, hipHostRegisterDefault));
  }
};
