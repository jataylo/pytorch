...
void createCublasHandle(hipblasHandle_t *handle) {
  TORCH_CUDABLAS_CHECK(hipblasCreate(handle));
}
...
size_t parseChosenWorkspaceSize() {
  ...
  hipDeviceProp_t* properties = at::cuda::getCurrentDeviceProperties();
...
at::DataPtr getNewWorkspace() {
  return c10::hip::HIPCachingAllocatorMasqueradingAsCUDA::get()->allocate(getChosenWorkspaceSize());
}

at::DataPtr getNewCUDABlasLtWorkspace() {
  return c10::hip::HIPCachingAllocatorMasqueradingAsCUDA::get()->allocate(getCUDABlasLtWorkspaceSize());
}

void setWorkspaceForHandle(hipblasHandle_t handle, c10::hip::HIPStreamMasqueradingAsCUDA stream) {
  ...
  // Fast path: check if workspace already exists
  {
    ...
    if (workspace_it != workspace.map.end()) {
      TORCH_CUDABLAS_CHECK(hipblasSetWorkspace(
  ...
  // were allocating)
  {
    ...
    TORCH_CUDABLAS_CHECK(
        hipblasSetWorkspace(handle, workspace_it->second.get(), workspace_size));
  }
}
...
#endif
  hipblasLtHandle_t handle = getCurrentCUDABlasLtHandle();
  auto stream = c10::hip::getCurrentHIPStreamMasqueradingAsCUDA();
  hipStream_t _stream = stream;
  auto key = std::make_tuple(static_cast<void *>(handle), static_cast<void *>(_stream));
...
hipblasHandle_t getCurrentCUDABlasHandle() {
  ...
  if (C10_UNLIKELY(!pctx)) {
    ...
    at::globalContext().getNVRTC().hipDevicePrimaryCtxRetain(&pctx, device);
  ...
  // This is because in CUDA 12.2+, the use of hipMallocAsync in cublas
  ...
  // all the memory and cublas's hipMallocAsync will return OOM
  ...
  if (!NoTF32Guard::should_disable_tf32() &&
    ...
    TORCH_CUDABLAS_CHECK(hipblasSetMathMode(handle, CUBLAS_TF32_TENSOR_OP_MATH));
  } else {
...
hipblasLtHandle_t getCurrentCUDABlasLtHandle() {
  ...
  return handle;
#else
  return reinterpret_cast<hipblasLtHandle_t>(getCurrentCUDABlasHandle());
#endif
}
