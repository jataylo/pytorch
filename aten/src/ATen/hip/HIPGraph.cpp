...
MempoolId_t graph_pool_handle() {
  ...
  // hipStreamGetCaptureInfo id_s in capture_begin.
...
/**
 * Note [Interaction with CUDA graph capture] in HIPCachingAllocator.cpp
...
void CUDAGraph::capture_begin(MempoolId_t pool/*=0*/, hipStreamCaptureMode capture_mode) {
  ...
  capture_dev_ = c10::hip::current_device();
  ...
  // Addendum: beginAllocateStreamToPool is now called before hipStreamBeginCapture to prevent an
  // autograd thread's free() call triggering an invalid hipEventRecord in the caching allocator
  // due to the capture status being updated _after_ a capture had already started.
  c10::hip::HIPCachingAllocatorMasqueradingAsCUDA::beginAllocateToPool(capture_dev_, mempool_id_, [this](hipStream_t stream) {
      hipStreamCaptureStatus status{};
      CaptureId_t stream_capture_id = 0;
      AT_CUDA_CHECK(hipStreamGetCaptureInfo(stream, &status, &stream_capture_id));
      return status == hipStreamCaptureStatus::hipStreamCaptureStatusActive && stream_capture_id == capture_id_;
  });

  // hipStreamCaptureModeGlobal is the most conservative option to
  // prevent potentially unsafe CUDA API calls during capture.  See
  // https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__STREAM.html#group__CUDART__STREAM_1g9d0535d93a214cbf126835257b16ba85
  AT_CUDA_CHECK(hipStreamBeginCapture(capture_stream_, capture_mode));

  hipStreamCaptureStatus status{};
  AT_CUDA_CHECK(hipStreamGetCaptureInfo(stream, &status, &capture_id_));
...
void CUDAGraph::capture_end() {
  ...
  AT_CUDA_CHECK(hipStreamEndCapture(capture_stream_, &graph_));

  c10::hip::HIPCachingAllocatorMasqueradingAsCUDA::endAllocateToPool(capture_dev_, mempool_id_);
  ...
  AT_CUDA_CHECK(hipGraphGetNodes(graph_, nullptr, &numCUDAGraphNodes));
  ...
  if (!keep_graph_) {
    ...
    if (!_cuda_graphs_debug) {
      AT_CUDA_CHECK(hipGraphDestroy(graph_));
    }
    has_graph_ = false;
  }
}
...
void CUDAGraph::instantiate() {
  ...
  if (has_graph_exec_) {
    ...
    AT_CUDA_CHECK(hipGraphExecDestroy(graph_exec_));
  }
  // In typical graph usage some tensors (e.g. the tensors used for graph IO) are not freed
  // between replays.
  // If Pytorch compiles and runs with a CUDA 11.4+ toolkit, there's a chance the allocator backend
  // is hipMallocAsync.
  // hipMallocAsync is generally graph-safe, but if some tensors are not freed between replays,
  // the graph's internal bookkeeping requires that we instantiate with
  // hipGraphInstantiateFlagAutoFreeOnLaunch. See
  // hipGraphLaunch
  // https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__GRAPH.html#group__CUDART__GRAPH_1g1accfe1da0c605a577c22d9751a09597
  // hipGraphInstantiateWithFlags
  // https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__GRAPH.html#group__CUDART__GRAPH_1ga2c652a24ba93e52b99a47bec0888233
  int version = 0;
  AT_CUDA_CHECK(hipDriverGetVersion(&version));
  if (version < 11040) {
    // Trailing NULL, NULL, 0 arguments were recommended by Cuda driver people,
    // who prefer not to report error message through these arguments moving forward
    // (they prefer return value, or errors on api calls internal to the capture)
    // ROCM appears to fail with HIP error: invalid argument
#if (defined(TORCH_HIP_VERSION) && TORCH_HIP_VERSION >= 12000) && !defined(USE_ROCM)
    AT_CUDA_CHECK(hipGraphInstantiate(&graph_exec_, graph_, hipGraphInstantiateFlagUseNodePriority));
#else
    AT_CUDA_CHECK(hipGraphInstantiate(&graph_exec_, graph_, NULL, NULL, 0));
#endif
...
//It will wait for the next sync operation. hipGraphInstantiateFlagAutoFreeOnLaunch will add async frees after graph launch.
  } else {
#if !defined(USE_ROCM)
    AT_CUDA_CHECK(hipGraphInstantiateWithFlags(&graph_exec_,
                                                graph_,
...
void CUDAGraph::replay() {
  ...
  AT_CUDA_CHECK(hipGraphLaunch(graph_exec_, at::hip::getCurrentHIPStreamMasqueradingAsCUDA()));
...
#if defined(TORCH_HIP_VERSION) || defined(USE_ROCM)
  if (_cuda_graphs_debug || keep_graph_) {
    TORCH_WARN("DEBUG: calling debug_dump()");
    if (has_graph_) {
      TORCH_WARN("DEBUG: calling hipGraphDebugDotPrint() with ", debug_path);
      C10_HIP_CHECK_WARN(hipGraphDebugDotPrint(graph_, debug_path.c_str(), hipGraphDebugDotFlagsVerbose)); // most verbose output
      if (!keep_graph_) {
        AT_CUDA_CHECK(hipGraphDestroy(graph_));
        has_graph_ = false;
      }
...
hipGraph_t CUDAGraph::raw_cuda_graph() {
  TORCH_CHECK(keep_graph_, "You cannot access the raw hipGraph_t instance unless CUDAGraph was initialized with keep_graph=true");
  TORCH_CHECK(has_graph_, "You cannot access the raw hipGraph_t instance until capture_end() has been called");
  return graph_;
}

hipGraphExec_t CUDAGraph::raw_cuda_graph_exec() {
  TORCH_CHECK(
      has_graph_exec_,
      "You cannot access the raw hipGraphExec_t instance until instantiate() has been called");
  return graph_exec_;
}
...
void CUDAGraph::reset() {
  ...
  if (capture_ended_) {
    ...
    c10::hip::HIPCachingAllocatorMasqueradingAsCUDA::releasePool(capture_dev_, mempool_id_);
    capture_ended_ = false;
  }
  if (has_graph_) {
    C10_HIP_CHECK_WARN(hipGraphDestroy(graph_));
    has_graph_ = false;
  }
  if (has_graph_exec_) {
    C10_HIP_CHECK_WARN(hipGraphExecDestroy(graph_exec_));
...
// We need to ensure all async operations finish before deleting the object.
  ...
  {
    AT_CUDA_CHECK(hipSetDevice(capture_dev_));
    AT_CUDA_CHECK(hipDeviceSynchronize());
  }
#endif
}
