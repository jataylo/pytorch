...
// add these definitions so that we can compile with HIP < 12.3
// borrowed from
...
enum ShareableHandleType : char {
  SHAREABLE_HIP_MALLOC = 'c',
  SHAREABLE_HIP_EXPANDABLE_SEGMENT = 'e'
};
...
(hipMemCreate) from the allocation of virtual address space (hipMemAddressReserve)
and the associate between them hipMemMap/hipMemSetAccess.
...
(hipMemGenericAllocationHandle_t) cannot be split up after it is mapped even if the
...
(hipDeviceEnablePeerAccess) do not work for memory allocated with hipMemMap.
...
struct ExpandableSegment {
  ExpandableSegment(
    ...
    C10_HIP_DRIVER_CHECK(DriverAPI::get()->hipMemAddressReserve_(
  ...
  SegmentRange map(SegmentRange range) {
    ...
    if (HIPAllocatorConfig::expandable_segments_handle_type() ==
        Expandable_Segments_Handle_Type::UNSPECIFIED) {
      HIPAllocatorConfig::set_expandable_segments_handle_type(
    ...
    for (auto i : c10::irange(begin, end)) {
      ...
      static const bool enable_ipc_handles =
          c10::utils::check_env("TORCH_HIP_EXPANDABLE_SEGMENTS_IPC")
      ...
      int flag = 0;
      C10_HIP_DRIVER_CHECK(DriverAPI::get()->hipDeviceGetAttribute_(
          &flag,
          CU_DEVICE_ATTRIBUTE_GPU_DIRECT_RDMA_WITH_HIP_VMM_SUPPORTED,
          device_));
      if (flag)
      ...
      auto status =
          DriverAPI::get()->hipMemCreate_(&handle, segment_size_, &prop, 0);
      if (status != hipSuccess) {
        if (status == hipErrorMemoryAllocation) {
          for (auto j : c10::irange(begin, i)) {
            ...
            C10_HIP_DRIVER_CHECK(DriverAPI::get()->hipMemRelease_(h.handle));
          }
          trimHandles();
          return rangeFromHandles(begin, begin);
        } else if (
            HIPAllocatorConfig::expandable_segments_handle_type() ==
            Expandable_Segments_Handle_Type::FABRIC_HANDLE) {
          ...
          // in practice, we can get hipErrorNotSupported or
          // hipErrorNotPermitted to be safe, any non out-of-memory error is
    ...
    for (auto i : c10::irange(begin, end)) {
      ...
      if (HIPAllocatorConfig::expandable_segments_handle_type() !=
        ...
        if (!handle.shareable_handle) {
          int fd = 0;
          C10_HIP_DRIVER_CHECK(DriverAPI::get()->hipMemExportToShareableHandle_(
...
#define SYS_pidfd_getfd 438
    ...
    if (HIPAllocatorConfig::expandable_segments_handle_type() !=
      ...
      TORCH_CHECK(
          ...
          "The kernel on this machine does not support the pidfd_open syscall needed to use IPC for HIP tensors when expandable_segments:True is set. "
      ...
      for (auto i : c10::irange(header.num_handles)) {
        ...
        if (myfd == -1) {
          ...
          for (auto& h : segment->handles_) {
            C10_HIP_DRIVER_CHECK(
                // NOLINTNEXTLINE(bugprone-unchecked-optional-access)
                DriverAPI::get()->hipMemRelease_(h.value().handle));
            h = std::nullopt;
          }
          TORCH_CHECK(
              err != ENOSYS,
              "The kernel on this machine does not support the pidfd_getfd syscall needed to use IPC for HIP tensors when expandable_segments:True is set. "
              "Consider using expandable_segments:False via torch.cuda.memory._set_allocator_settings('expandable_segments:False') for this allocation.");
          TORCH_CHECK(false, "pidfd_getfd: ", c10::utils::str_error(err));
        }
        hipMemGenericAllocationHandle_t handle = 0;
        C10_HIP_DRIVER_CHECK(DriverAPI::get()->hipMemImportFromShareableHandle_(
            &handle,
  ...
  ~ExpandableSegment() {
    ...
    C10_HIP_DRIVER_CHECK(DriverAPI::get()->hipMemAddressFree_(
 ...
 private:
  ...
  void unmapHandles(size_t begin, size_t end) {
    ...
    if (stream_) {
      ...
      hip::HIPGuard device_guard(device_);
    ...
    for (auto i : c10::irange(begin, end)) {
      ...
      C10_HIP_DRIVER_CHECK(DriverAPI::get()->hipMemUnmap_(
  ...
  std::optional<hipStream_t> stream_;
  hipDeviceptr_t ptr_{};
...
// HIP graphs helper
struct PrivatePool {
  explicit PrivatePool(MempoolId_t id, HIPAllocator* allocator = nullptr)
      : id(std::move(id)),
...
hipError_t allocPrimitive(void** ptr, size_t size, AllocParams& p) {
  if (p.pool->owner_PrivatePool && p.pool->owner_PrivatePool->allocator()) {
    *ptr = p.pool->owner_PrivatePool->allocator()->raw_alloc(size);
    return *ptr ? hipSuccess : hipErrorMemoryAllocation;
  } else {
    return C10_HIP_ERROR_HANDLED(hipMalloc(ptr, size));
  }
}

hipError_t hipMallocMaybeCapturing(void** ptr, size_t size, AllocParams& p) {
  if (at::cuda::currentStreamCaptureStatusMayInitCtx() ==
    ...
    at::cuda::HIPStreamCaptureModeGuard g{hipStreamCaptureModeRelaxed};
    return allocPrimitive(ptr, size, p);
  }
}
...
static std::string reportProcessMemoryInfo(const hipDeviceProp_t& prop) {
...
class DeviceCachingAllocator {
 private:
  ...
  // Map of blocks whose freeing is deferred until after HIP graph capture.
  ...
  //     ends.
  ska::flat_hash_map<Block*, std::vector<hipGraphNode_t>> deferred_blocks;
  ...
  struct GraphReuseContext {
    ska::flat_hash_map<hipStream_t, ska::flat_hash_set<hipGraphNode_t>>
        visited;
  };
  ...
  std::unordered_map<Block*, stream_set> block_to_hipgraph_stream_uses;
 ...
 public:
  ...
  explicit DeviceCachingAllocator(c10::DeviceIndex id)
    ...
    C10_HIP_CHECK(hipGetDeviceProperties(&device_prop, id));

    setMemoryFraction(HIPAllocatorConfig::per_process_memory_fraction());
  ...
  Block* malloc(size_t orig_size, hipStream_t stream) {
    ...
    if (C10_LIKELY(captures_underway.empty())) {
      ...
      if (HIPAllocatorConfig::graph_capture_record_stream_reuse()) {
    ...
    bool is_expandable_segments_active =
        HIPAllocatorConfig::expandable_segments() && !active_user_pool;
    AllocParams params(
        device_id,
        size,
        stream,
    ...
    if (!block_found && params.err == hipErrorMemoryAllocation) {
  ...
  Block* alloc_found_block(
    ...
    auto allocated_bytes_gauge =
        STATIC_GAUGE(pytorch.HIPCachingAllocator.allocated_bytes);
  ...
  struct CaptureInfo {
    ...
    const hipGraphNode_t* terminals{nullptr};
    size_t num_terminals{0};
    hipStreamCaptureStatus status{hipStreamCaptureStatusNone};
  };

  CaptureInfo stream_get_capture_info(hipStream_t stream) {
    CaptureInfo info{};
#if (defined(TORCH_HIP_VERSION) && TORCH_HIP_VERSION >= 13000)
    C10_HIP_CHECK(hipStreamGetCaptureInfo(
        stream,
        &info.status,
        &info.capture_id,
        &info.graph,
        ...
        &info.num_terminals));
#else
    C10_HIP_CHECK(hipStreamGetCaptureInfo_v2(
        stream,
        &info.status,
        &info.capture_id,
        &info.graph,
...
#endif
    TORCH_INTERNAL_ASSERT(
        info.status != hipStreamCaptureStatusInvalidated,
        "Invalid stream capture status");

    return info;
  }

  // Record "free marker" of the HIP graph for all streams that
  ...
  std::vector<hipGraphNode_t> record_free_markers(Block* block) {
    // Is is possible to have the same marker recorded multiple times, so we use
    // a set to avoid duplicates
    ska::flat_hash_set<hipGraphNode_t> markers;
    hipGraph_t owning_graph = nullptr;

    auto try_record = [&](hipStream_t s) -> bool {
      auto info = stream_get_capture_info(s);
      if (info.status == hipStreamCaptureStatusNone) {
    ...
    return std::vector<hipGraphNode_t>(markers.begin(), markers.end());
  }

  // Returns the set of "reusable" free markers in the current
  // HIP graph capture. A free marker is considered reusable if it is a
  ...
  void update_visited(
      ...
      ska::flat_hash_set<hipGraphNode_t>& visited) {
    // This is the versioned hipGraphNodeGetDependencies helper function.
    auto node_get_dependencies =
        [](hipGraphNode_t n, hipGraphNode_t* deps, size_t* count) -> void {
#if (defined(TORCH_HIP_VERSION) && TORCH_HIP_VERSION >= 13000)
      if (deps == nullptr) {
        C10_HIP_CHECK(hipGraphNodeGetDependencies(n, deps, nullptr, count));
      } else {
        SmallVector<hipGraphEdgeData> edgeData;
        edgeData.resize(*count);
        C10_HIP_CHECK(
            hipGraphNodeGetDependencies(n, deps, edgeData.data(), count));
      }
#else
      C10_HIP_CHECK(hipGraphNodeGetDependencies(n, deps, count));
#endif
    };

    // Helper to retrieve all parent nodes (dependencies) of a given node.
    auto get_parents =
        [&](hipGraphNode_t node) -> std::vector<hipGraphNode_t> {
      size_t count = 0;

      node_get_dependencies(node, nullptr, &count);
      std::vector<hipGraphNode_t> out(count);
      if (count) {
  ...
  // A block is considered reusable during HIP graph capture if every free
  ...
  void free_safe_blocks_in_capture(
    ...
    if (info.status == hipStreamCaptureStatusNone || info.num_terminals == 0) {
      return;
    }
    ...
    if (!block->stream_uses.empty()) {
      if (C10_UNLIKELY(!captures_underway.empty())) {
        if (HIPAllocatorConfig::graph_capture_record_stream_reuse()) {
          ...
          deferred_blocks.emplace(block, std::vector<hipGraphNode_t>{});
        }
      } else {
  ...
  ShareableHandle shareIpcHandle(Block* block) {
    ...
    if (!block->expandable_segment_) {
      ...
      C10_HIP_CHECK(hipIpcGetMemHandle(&handle, base_block->ptr));
      ss.write(reinterpret_cast<const char*>(&handle), HIP_IPC_HANDLE_SIZE);
    } else {
      ss.put(SHAREABLE_HIP_EXPANDABLE_SEGMENT);
  ...
  void recordStream(Block* block, hip::HIPStream stream) {
    ...
    if (C10_UNLIKELY(!captures_underway.empty())) {
      block_to_hipgraph_stream_uses[block].insert(stream);
    }
  }
  ...
  std::vector<SegmentInfo> snapshot(MempoolId_t mempool_id) {
    ...
    if (mempool_id.first != 0 || mempool_id.second != 0) {
      ...
      // all the blocks in the HIPCachingAllocator (as returned by
      // get_all_blocks).
  ...
  void createOrIncrefPool(MempoolId_t mempool_id, HIPAllocator* allocator) {
  ...
  // Called by HIPGraph::reset and MemPool::~MemPool()
 ...
 private:
  ...
  void create_or_incref_pool(
      MempoolId_t mempool_id,
      HIPAllocator* allocator = nullptr) {
    auto it = graph_pools.find(mempool_id);
    if (it == graph_pools.end()) {
      // mempool_id does not reference an existing pool.
      // Make a new pool for HIPGraph capture or torch.cuda.use_mem_pool
      ...
      // mempool_id references an existing pool, which the current HIPGraph
  ...
  bool map_block(
    ...
    auto reserved_bytes_gauge =
        STATIC_GAUGE(pytorch.HIPCachingAllocator.reserved_bytes);
  ...
  bool alloc_block(
    ...
    if (allowed_memory_maximum.has_value() &&
      ...
      if (HIPAllocatorConfig::release_lock_on_hipmalloc()) {
        ...
        lock.unlock();
        p.err = hipMallocMaybeCapturing(&ptr, size, p);
      } else {
        p.err = hipMallocMaybeCapturing(&ptr, size, p);
      }
      if (HIPAllocatorConfig::release_lock_on_hipmalloc()) {
  ...
  void unmap_block(
    ...
    if (block->pool->owner_PrivatePool) {
      // The hipFreed block belonged to a HIP graph's PrivatePool.
      TORCH_INTERNAL_ASSERT(
          block->pool->owner_PrivatePool->hipMalloc_count > 0);
      block->pool->owner_PrivatePool->hipMalloc_count--;
    }
  ...
  void remove_hipgraph_stream_uses(Block* block) {
    // remove stream uses added during cudagraph capture
    // (i.e., block->stream_uses - block->cudagraph_stream_uses)
    if (C10_UNLIKELY(
            block_to_hipgraph_stream_uses.find(block) !=
            block_to_hipgraph_stream_uses.end())) {
      stream_set streams(std::move(block->stream_uses));
      AT_ASSERT(block->stream_uses.empty());
      for (auto& stream : streams) {
        if (block_to_hipgraph_stream_uses[block].find(stream) ==
            block_to_hipgraph_stream_uses[block].end()) {
          block->stream_uses.insert(stream);
        }
      }
      block_to_hipgraph_stream_uses.erase(block);
    }
  }
  ...
  void insert_events_deferred_until_no_capture(
    ...
    if (C10_UNLIKELY(!deferred_blocks.empty())) {
      for (auto& [block, inserted_empty_nodes] : deferred_blocks) {
        ...
        // Events in HIP Programming Guide).
        remove_hipgraph_stream_uses(block);
        insert_events(block);
        if (block->event_count == 0) {
          free_block(block, context);
        }
...
static bool forceUncachedAllocator() {
  // Allow either HIP or HIP name for env var for maximum user comfort
  // the HIP env var avoids being hipified in hip_to_hip_mappings.py
  static auto has_hip_env =
  ...
  static bool force_uncached = has_hip_env || has_rocm_env;
...
static void* uncached_allocate(size_t size) {
  ...
  if (C10_UNLIKELY(interp)) {
    (*interp)->trace_gpu_memory_allocation(
        c10::kHIP, reinterpret_cast<uintptr_t>(devPtr));
  }
  return devPtr;
}
...
static void uncached_delete(void* ptr) {
  ...
  if (C10_UNLIKELY(interp)) {
    (*interp)->trace_gpu_memory_deallocation(
        c10::kHIP, reinterpret_cast<uintptr_t>(ptr));
  }
...
class NativeCachingAllocator : public HIPAllocator {
 ...
 public:
  ...
  void free(void* ptr) {
    ...
    if (C10_UNLIKELY(interp)) {
      (*interp)->trace_gpu_memory_deallocation(
          c10::kHIP, reinterpret_cast<uintptr_t>(block->ptr));
    }
  ...
  SnapshotInfo snapshot(MempoolId_t mempool_id) override {
    ...
    md.pinned_num_register_threads =
        HIPAllocatorConfig::pinned_num_register_threads();
    md.expandable_segments = HIPAllocatorConfig::expandable_segments();
    md.release_lock_on_malloc =
        HIPAllocatorConfig::release_lock_on_hipmalloc();
    md.pinned_use_host_register =
        HIPAllocatorConfig::pinned_use_hip_host_register();
    md.last_allocator_settings =
        AcceleratorAllocatorConfig::last_allocator_settings();
    md.graph_capture_record_stream_reuse =
        HIPAllocatorConfig::graph_capture_record_stream_reuse();
  ...
  void createOrIncrefPool(
      c10::DeviceIndex device,
      MempoolId_t mempool_id,
      HIPAllocator* allocator) override {
    assertValidDevice(device);
  ...
  // In HIP IPC, sender sends a tensor to receiver via shareIPCHandle,
  // getIpcDevPtr is called by the receiving process to map the HIP memory from
  // the sending process into its own address space.

  // When allocated with hipMalloc we use the cudaIPCMemHandle_t APIs.
  ...
  // When using hipMemCreate, via expandable segments, we use
  // hipMemExportToShareableHandle to create a file descriptor that can be sent
  ...
  struct MemHandleCacheEntry {
    MemHandleCacheEntry(
        ...
        : device_(device) {
      int type = SHAREABLE_HIP_MALLOC;
      std::istringstream ss(handle);
      if (handle.size() != HIP_IPC_HANDLE_SIZE) {
        auto version = ss.get();
        TORCH_CHECK(
        ...
        // SHARABLE_HIP_MALLOC
      if (type == SHAREABLE_HIP_MALLOC) {
        hipIpcMemHandle_t hip_handle;
        ss.read(reinterpret_cast<char*>(&hip_handle), HIP_IPC_HANDLE_SIZE);
        C10_HIP_CHECK(hipIpcOpenMemHandle(
            &hip_ipc_ptr_, hip_handle, hipIpcMemLazyEnablePeerAccess));
      } else if (type == SHAREABLE_HIP_EXPANDABLE_SEGMENT) {
        expandable_segment_ =
    ...
    // stored raw hip_ipc_ptr_ handles.
    void clear() {
      if (hip_ipc_ptr_) {
        hip::HIPGuard device_guard(device_);
        C10_HIP_CHECK(hipIpcCloseMemHandle(hip_ipc_ptr_));
        hip_ipc_ptr_ = nullptr;
      }
    ...
    c10::DeviceIndex device_;
    ExpandableSegment* expandable_segment_{nullptr};
    void* hip_ipc_ptr_{nullptr}; // nullptr if expandable_segment_ is not null
...
struct BackendStaticInitializer {
  // Parses the environment configuration for HIP/ROCm allocator backend at
  // load time. This duplicates some logic from HIPAllocatorConfig to ensure
...
#ifdef USE_ROCM
    // convenience for ROCm users to allow either HIP or HIP env var
    if (!val.has_value()) {
...
// Register this HIP allocator as the HIP allocator to allow it to work
// with both c10::GetAllocator(kHIP) and c10::getDeviceAllocator(kHIP)
// APIs. We don't perform this masquerading inside
// HIPAllocatorMasqueradingAsHIP because it needs to happen during static
// initialization, and doing so there may introduce static initialization
// order (SIOF) issues.
#define HIP_MASQUERADING_AS_HIP "cuda"
    at::SetAllocator(c10::Device(HIP_MASQUERADING_AS_HIP).type(), r, 0);
    allocator.store(r);
#undef HIP_MASQUERADING_AS_HIP
  }
};
