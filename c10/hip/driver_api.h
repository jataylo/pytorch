...
#define C10_HIP_DRIVER_CHECK_GOTO(EXPR, NEXT)                             \
  do {                                                                     \
    hipError_t __err = EXPR;                                                 \
    if (__err != hipSuccess) {                                           \
      const char* err_str;                                                 \
      hipError_t get_error_str_err [[maybe_unused]] =                        \
          c10::hip::DriverAPI::get()->hipDrvGetErrorString_(__err, &err_str); \
      if (get_error_str_err != hipSuccess) {                             \
        TORCH_WARN("HIP driver error: unknown error");                    \
      } else {                                                             \
        TORCH_WARN("HIP driver error: ", err_str);                        \
      }                                                                    \
      goto NEXT;                                                           \
    }                                                                      \
  } while (0)
