...
constexpr unsigned int kDefaultFlags = hipStreamNonBlocking;
...
// the destruction.
#if !defined(USE_ROCM)
// HIP-only: used to initializes the stream pools (once)
...
static c10::once_flag
    stream_flags[c10::hip::max_compile_time_stream_priorities]
...
// lightweight as HIP streams; the pooling strategy can affect performance.
...
void initSingleStream(int p, DeviceIndex device_index, int i) {
  ...
  if (C10_UNLIKELY(interp)) {
    (*interp)->trace_gpu_stream_creation(
        c10::kHIP, reinterpret_cast<uintptr_t>(stream));
...
#if !defined(USE_ROCM)
  // See Note [HIP Lazy Streams]
  // HIP-only: Initializes the stream pools (once)
  c10::call_once(
