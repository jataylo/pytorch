...
#define PYTORCH_TOKEN2 "hipMallocAsync"
  ...
  used_hipMallocAsync = (tokenizer[i] == PYTORCH_TOKEN1);
  TORCH_INTERNAL_ASSERT(
      tokenizer[i] == get()->name(),
      "Allocator backend parsed at runtime != "
      "allocator backend parsed at load time, ",
...
void HIPAllocatorConfig::parseArgs(const std::string& env) {
  ...
  for (size_t i = 0; i < tokenizer.size(); i++) {
    ...
    if (key == "backend") {
      i = parseAllocatorConfig(tokenizer, i, used_hipMallocAsync);
    } else if (
...
size_t HIPAllocatorConfig::parseGraphCaptureRecordStreamReuse(
    const c10::CachingAllocator::ConfigTokenizer& tokenizer,
    size_t i) {
  tokenizer.checkToken(++i, ":");
...
double HIPAllocatorConfig::parsePerProcessMemoryFraction(
    const c10::CachingAllocator::ConfigTokenizer& tokenizer,
    size_t i) {
  tokenizer.checkToken(++i, ":");
...
size_t HIPAllocatorConfig::parsePinnedReserveSegmentSize(
    const c10::CachingAllocator::ConfigTokenizer& tokenizer,
    size_t i) {
  tokenizer.checkToken(++i, ":");
  size_t val2 = tokenizer.toSizeT(++i);
