...
struct TraceEntry {
  enum Action {
    ...
    SEGMENT_MAP, // a call to hipMemMap (used with expandable_segments)
...
struct StreamSegmentSize {
  StreamSegmentSize(hipStream_t s, bool small, size_t sz)
...
class HIPAllocator : public DeviceAllocator {
 public:
  ...
  // Keep for BC only
  virtual void recordStream(const DataPtr& ptr, HIPStream stream) = 0;
  ...
  //  with P2P Disabled:
  //                       hipMalloc hipMallocAsync/hipMemMap
  ...
  std::pair<size_t, size_t> getMemoryInfo(c10::DeviceIndex device) override {
    c10::DeviceGuard device_guard({at::kHIP, device});
    size_t free = 0;
    size_t total = 0;
    C10_HIP_CHECK(hipMemGetInfo(&free, &total));
    return {free, total};
  }
};
...
inline void createOrIncrefPool(
    c10::DeviceIndex device,
    MempoolId_t mempool_id,
    HIPAllocator* allocator_ptr = nullptr) {
