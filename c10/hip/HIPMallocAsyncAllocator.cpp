...
inline void lazy_init_device(c10::DeviceIndex device) {
  if (!devs_initialized_flags[device]) {
    ...
    C10_HIP_CHECK(hipDeviceGetDefaultMemPool(&mempool, device));
    uint64_t threshold = UINT64_MAX;
    C10_HIP_CHECK(hipMemPoolSetAttribute(
...
struct HipMallocAsyncAllocator : public HIPAllocator {
  ...
  std::vector<StreamSegmentSize> getExpandableSegmentSizes(
    ...
    TORCH_CHECK(
        false,
        "HIPMallocAsyncAllocator does not yet support getExpandableSegmentSizes.");
  }
  ...
  void emptyCache(/*unused*/ MempoolId_t mempool_id) override {
    ...
    for (int dev = 0; dev < device_count; dev++) {
      if (devs_initialized_flags[dev]) {
        HIPGuard g(static_cast<c10::DeviceIndex>(dev));

        hipMemPool_t mempool = nullptr;
        C10_HIP_CHECK(hipDeviceGetDefaultMemPool(&mempool, dev));
        C10_HIP_CHECK(hipDeviceSynchronize());
        C10_HIP_CHECK(hipMemPoolTrimTo(mempool, 0));
      }
    }
  }
  ...
  void cacheInfo(c10::DeviceIndex device, size_t* maxWorkspaceGuess) override {
    ...
    while (true) {
      ...
      if (err == hipSuccess) {
        C10_HIP_CHECK(hipFreeAsync(dummy, stream));
  ...
  ShareableHandle shareIpcHandle(void* handle) override {
    TORCH_CHECK(
        false,
        "hipMallocAsync does not yet support shareIpcHandle. "
  ...
  DeviceStats getDeviceStats(c10::DeviceIndex device) override {
    ...
    if (devs_initialized_flags[device]) {
      ...
      hipMemPool_t mempool = nullptr;
      C10_HIP_CHECK(hipDeviceGetDefaultMemPool(&mempool, device));
      C10_HIP_CHECK(hipMemPoolGetAttribute(
  ...
  void enablePeerAccess(c10::DeviceIndex dev, c10::DeviceIndex dev_to_access)
    ...
    hipMemPool_t mempool = nullptr;
    C10_HIP_CHECK(hipDeviceGetDefaultMemPool(&mempool, dev_to_access));
    hipMemAccessDesc desc = {};
    desc.location.type = hipMemLocationTypeDevice;
    // NOLINTNEXTLINE(bugprone-signed-char-misuse)
    desc.location.id = dev;
    desc.flags = hipMemAccessFlagsProtReadWrite;
    C10_HIP_CHECK(hipMemPoolSetAccess(mempool, &desc, 1 /* numDescs */));
  }
...
static HipMallocAsyncAllocator device_allocator;
