...
#define LOOKUP_LIBHIP_ENTRY_WITH_VERSION_REQUIRED(name, version)            \
  r.name##_ = reinterpret_cast<decltype(&name)>(get_symbol(#name, version)); \
  TORCH_INTERNAL_ASSERT(r.name##_, "Can't find ", #name);
  C10_LIBHIP_DRIVER_API_REQUIRED(LOOKUP_LIBHIP_ENTRY_WITH_VERSION_REQUIRED)
#undef LOOKUP_LIBHIP_ENTRY_WITH_VERSION_REQUIRED
...
#define LOOKUP_LIBHIP_ENTRY_WITH_VERSION_OPTIONAL(name, version) \
  r.name##_ = reinterpret_cast<decltype(&name)>(get_symbol(#name, version));
  C10_LIBHIP_DRIVER_API_OPTIONAL(LOOKUP_LIBHIP_ENTRY_WITH_VERSION_OPTIONAL)
#undef LOOKUP_LIBHIP_ENTRY_WITH_VERSION_OPTIONAL
...
void* get_symbol(const char* name, int version) {
  ...
  // HIP 12.5+ supports version-based lookup
#if defined(TORCH_HIP_VERSION) && (TORCH_HIP_VERSION >= 12050)
  if (auto st = cudaGetDriverEntryPointByVersion(
          name, &out, version, cudaEnableDefault, &qres);
      st == hipSuccess && qres == cudaDriverEntryPointSuccess && out) {
    return out;
  }
#endif

  // As of HIP 13, this API is deprecated.
